...
RSpec.describe SidebarHelper, type: :helper do
  ...
  describe "#allowed_sidebar_items" do
    ...
    it "CMD (non-ZM): 6 items — không có nhập liệu, unit_config" do
      cmd = create(:user, :commander, unit: other_unit)
      items = items_for(cmd)
      expect(items.size).to eq(6)
      expect(items).to include(:dashboard, :billing, :history, :contact_points, :blocks, :groups)
      expect(items).not_to include(:meter_entries, :unit_config, :electricity_supply)
    end

    it "CMD-ZM: 8 items — thêm zones, pump_allocations" do
      cmd_zm = create(:user, :commander, unit: manager_unit)
      items = items_for(cmd_zm)
      expect(items.size).to eq(8)
      expect(items).to include(:zones, :pump_allocations)
      expect(items).not_to include(:meter_entries, :unit_config, :electricity_supply)
    end
  end
end
