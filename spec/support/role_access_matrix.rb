...
# each of the 7 real roles) AND the pure policy functions that the guardrail spec
...
module RoleAccessMatrix
  # The 7 real roles (docs/V2_HANH_VI_HE_THONG.md mục 1). UA-ZM / CMD-ZM are the
  # zone-manager variants of unit_admin / commander. Defined once, here.
  ROLES = %i[sa dc ua_zm ua cmd_zm cmd tech].freeze

  # Human-facing labels for the 7 roles (test descriptions / failure messages).
  ...
  PAGES = {
    ...
    "dashboard"          => { category: "Xem kết quả", path: :dashboard_path,
                             expect: { sa: :ok, dc: :ok, ua_zm: :ok, ua: :ok, cmd_zm: :ok, cmd: :ok, tech: :redirect } },
    ...
    "electricity_supply" => { category: "Nhập liệu", path: :electricity_supply_path,
                             expect: { sa: :ok, dc: :ok, ua_zm: :ok, ua: :redirect, cmd_zm: :ok, cmd: :redirect, tech: :redirect } },
    ...
    "zones"              => { category: "Thiết lập", path: :zones_path,
                             expect: { sa: :ok, dc: :ok, ua_zm: :redirect, ua: :redirect, cmd_zm: :redirect, cmd: :redirect, tech: :redirect } },
    ...
    "users"              => { category: "Hệ thống", path: :users_path,
                             expect: { sa: :ok, dc: :redirect, ua_zm: :redirect, ua: :redirect, cmd_zm: :redirect, cmd: :redirect, tech: :ok } },
    "audit_logs"         => { category: "Hệ thống", path: :audit_logs_path,
                             expect: { sa: :ok, dc: :ok, ua_zm: :redirect, ua: :redirect, cmd_zm: :redirect, cmd: :redirect, tech: :ok } },
    "backups"            => { category: "Hệ thống", path: :backups_path,
                             expect: { sa: :redirect, dc: :redirect, ua_zm: :redirect, ua: :redirect, cmd_zm: :redirect, cmd: :redirect, tech: :ok } }
  }.freeze
  ...
  # Pages whose expectation hash does not cover all 7 roles.
