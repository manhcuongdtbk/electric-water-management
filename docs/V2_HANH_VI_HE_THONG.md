...
1. [7 vai trò thực tế](#1-7-vai-trò-thực-tế)
...
User model có 5 enum values (`system_admin`, `division_commander`, `unit_admin`, `commander`, `technician`), nhưng hệ thống có **7 vai trò** thực tế vì `unit_admin` và `commander` mỗi role chia thành 2 variant tùy thuộc đơn vị có quản lý khu vực hay không:
...
### Hành vi xuyên kỳ mỗi loại đầu mối
...
| Loại | Data mỗi kỳ khi tạo | Cleanup khi xóa | Kế thừa khi mở kỳ mới |
|---|---|---|---|
...
- **Nhập liệu:** Chỉ sửa số liệu mỗi kỳ (meter_readings, personnel_entries, unit_configs, other_deductions, pump_allocations, main_meter_readings).
...
| DC | Tất cả sinh hoạt + công cộng | Tất cả bơm nước | Không (disabled) |
...
Cấu hình tỷ lệ công cộng đơn vị + cột "Khác" (other_deductions) mỗi đầu mối **sinh hoạt**. Đầu mối công cộng không có other_deductions.
...
| DC | Chọn unit bất kỳ | Unit đó | Nếu unit là quản lý khu vực | Không (chỉ xem, inputs disabled) |
...
|---|---|---|---|
| SA | Tất cả | Có | Cho sửa khi kỳ cũ mở lại (không có StructureChangeGuard — pump_allocations là data mỗi kỳ) |
| DC | Tất cả | Không (chỉ xem) | — |
...
| DC | Tất cả main_meters | Không (disabled) | — |
...
| Vai trò | Nội dung |
|---|---|
| SA | Bảng đơn vị (thâm điện, thành tiền, trạng thái nhập liệu) + bảng khu vực (tổng sử dụng điện công cộng + bơm nước mỗi zone) + cảnh báo |
| DC | Tổng quan toàn hệ thống (như SA), chỉ xem |
...
Hệ thống **không lưu** entity bị xóa ở kỳ nào (`discarded_at` là timestamp, không phải period reference). Thay vào đó, **data mỗi kỳ tự lọc:**
...
| Block/Group | Nullify block_id/group_id trên children | — (không có data mỗi kỳ) |
...
| Ngữ cảnh | Dùng | Lý do |
|---|---|---|
| **Query hiển thị data mỗi kỳ** (billing, meter_entries, dashboard, ...) | Không dùng `.kept` — data mỗi kỳ tự lọc | Kỳ cũ cần hiện entity đã xóa |
...
> Hành vi runtime cho 3 tính năng 1.2.0 (**đã triển khai**, merge vào `develop`). Chi tiết + lý do ở spec; data model ở `V2_THIET_KE_HE_THONG` mục "Thiết kế bổ sung — milestone 1.2.0".
...
- Mục 9: sửa ghi chú "chưa triển khai phiên này" → "đã triển khai" — cả 3 tính năng 1.2.0 đã merge vào develop.
