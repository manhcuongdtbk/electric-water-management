...
**Tổn hao mỗi đầu mối sinh hoạt** (tổng tổn hao các công tơ trong đầu mối):
...
### EN-KV1-SUMMARY — Tổng hợp mỗi đầu mối sinh hoạt `[TỰ ĐỘNG]`
...
**Phân bổ theo đối tượng** (không có phần trăm cố định; toàn bộ theo hệ số):
...
### EN-KV2-SUMMARY — Tổng hợp mỗi đầu mối sinh hoạt `[TỰ ĐỘNG]`
...
Sau bước này, "Lái xe" ở trạng thái discarded (`discarded_at` có giá trị), nhưng dữ liệu mỗi kỳ của nó ở kỳ N-1 vẫn còn (không bị cleanup vì lúc xóa ở kỳ N, chỉ dữ liệu kỳ N bị hard delete). Đây là kịch bản xuyên kỳ quan trọng nhất theo `V2_CHIEU_TEST.md` chiều 4.
...
- **Kết quả mong đợi:**
  ...
  - Đối chiếu: nếu SA đổi dropdown kỳ về N-2 (kỳ đang mở lại), nút Tính toán lại **bật** (sửa được dữ liệu mỗi kỳ ở trạng thái C). Đây là phân biệt cốt lõi: recalculate của kỳ đang mở (N-2) hoạt động, kỳ đóng (N-1) thì không.
...
Suite này instance hóa Nhóm 6 (giao điểm chiều 11 × 1 × 5). Kiểm ba đường nhận dữ liệu mỗi kỳ (kế thừa khi mở kỳ mới, tạo giữa kỳ qua after_create, kỳ đầu tiên) theo `V2_HANH_VI_HE_THONG.md` mục 6. Dùng Khu vực 1.
...
Tài liệu này **không chép lại** ma trận chiều 3; mỗi trang ghi phần giới thiệu ngắn (route + lớp bảo vệ + trỏ tới chiều 3) rồi đầu ra cụ thể mỗi vai trò. Mã kịch bản: `TR-<trang>-<vaitro>` (ví dụ TR-billing-SA, TR-meter_entries-UAZM, TR-users-TECH).
...
- **Số mục sidebar theo vai trò:** xem bảng 4.0 bên dưới (nguồn: `V2_THIET_KE_HE_THONG.md` mục "Sidebar theo vai trò").
...
- Thẻ loại kiểm thử ghi mỗi kịch bản theo mục 0.2.2.

### 4.0. Tổng quan sidebar theo vai trò `[CẢ HAI]`

Trước khi đi vào từng trang, bảng này chốt số mục sidebar và danh mục mục hiển thị mỗi vai trò (nguồn: `V2_THIET_KE_HE_THONG.md` mục "Sidebar theo vai trò"). Mọi trang phía dưới giả định sidebar đã đúng theo bảng này.
...
- **Chiều liên quan:** chiều 2 (7 vai trò), chiều 3 (Tổng quan), chiều 8 (trạng thái tính toán), chiều 9 (cảnh báo mỗi phạm vi).
...
**Route:** `/history` (history#show). **Lớp bảo vệ:** BusinessRoleRequired (chiều 3, hàng Tra cứu lịch sử). Thao tác: Xem kỳ cũ, so sánh 2 kỳ, xem theo khoảng. 6 vai trò nghiệp vụ xem được; TECH chặn. Phạm vi dữ liệu mỗi vai trò **giống billing** (SA toàn bộ, UA-ZM đơn vị + khu vực trực tiếp, UA đơn vị mình). Khác biệt với billing: trang so sánh **luôn hiện cả hai cột Khu vực và Đơn vị** (so sánh kỳ cần đủ ngữ cảnh — theo CLAUDE.md); dropdown khu vực/đơn vị khi xem kỳ cũ dùng `with_discarded` để hiện entity đã xóa.
...
**Vị trí phân cấp hiển thị (chiều 10):** danh sách hiện cột Khối/Nhóm theo vị trí — Ban Tác huấn (khối + nhóm), Văn thư (khối), Kho vật tư (trực tiếp đơn vị), Quân y (nhóm không khối — Khu vực 2), Chỉ huy khu vực (thuộc khu vực, cột Đơn vị trống). Trạng thái kỳ cũ mở lại (C): StructureChangeGuard chặn tạo/xóa/đổi tên, chỉ sửa số liệu mỗi kỳ.
...
**Route:** `/unit_config` (unit_config#show). **Lớp bảo vệ:** BusinessRoleRequired + PeriodGuard (chiều 3). Cấu hình tỷ lệ công cộng đơn vị (unit_public_rate) + cột "Khác" (other_deductions) mỗi đầu mối **sinh hoạt**.
...
**Route:** `/pricing` (pricing#show). **Lớp bảo vệ:** SettingsAccessGuard (`require_system_admin!`) + authorize! (chiều 3). Đơn giá mỗi kỳ + mở/đóng/mở lại kỳ. **Chỉ SA toàn quyền**; non-SA bị **chặn** (page-level guard, sidebar cũng ẩn mục).
...
- **Các bước:**
  ...
  3. Thử sửa **số liệu mỗi kỳ**: chỉ số công tơ (`/meter_entries`), quân số, cấu hình đơn vị (`/unit_config`), phân bổ bơm nước (`/pump_allocations`).
- **Kết quả mong đợi:**
  - Mở lại thành công: `period` tháng 5 `closed = false`, StructureChangeGuard active.
  - Thay đổi cấu trúc bị chặn: StructureChangeGuard chặn + flash "đang mở kỳ cũ, chỉ cho phép sửa số liệu". Lý do: thực thể cấu trúc không có bản sao riêng mỗi kỳ, thay đổi sẽ ảnh hưởng mọi kỳ (nghiệp vụ mục 12).
  - Sửa số liệu mỗi kỳ **được phép**: cập nhật reading_end, quân số, unit_public_rate, phân bổ bơm nước của kỳ tháng 5 — chỉ ảnh hưởng kỳ tháng 5, không ảnh hưởng kỳ khác. Bấm Tính toán lại được (recalculate hoạt động ở trạng thái C).
- **Tham chiếu:** `V2_CHIEU_TEST.md` chiều 1 (trạng thái C), U-update (kỳ cũ mở lại: sửa data mỗi kỳ được, sửa cấu trúc bị chặn), Thao tác đặc biệt (Mở lại kỳ cũ); `V2_HANH_VI_HE_THONG.md` mục 3 (trạng thái C); nghiệp vụ mục 12.
...
- **Kết quả mong đợi (theo `V2_HANH_VI_HE_THONG.md` mục 3):**
  ...
  | Trạng thái | Cách đạt | Cấu trúc | Số liệu mỗi kỳ | Tính toán lại |
  |---|---|---|---|---|
...
- **Kết quả mong đợi (theo nghiệp vụ mục 23.1, 27.2, 27.4):**
  ...
  - Xóa đầu mối/công tơ có dữ liệu kỳ cũ: cho phép; cleanup dữ liệu kỳ đang mở (hard delete), dữ liệu kỳ cũ giữ nguyên (xem GD2 chi tiết mỗi loại đầu mối).
...
- **Phân quyền** (GD3, TR-*, phân quyền chung): `role_access_matrix_spec`, `business_role_required_integration_spec`, `dimension_coverage_spec` + request spec mỗi trang — kiểm 7 vai trò thấy/không thấy/bị chặn đúng.
- **Validation** (VH-validation): model spec mỗi model (`meter_reading_spec`, `pump_allocation_spec`, `unit_config_spec`, `personnel_entry_spec`, `other_deduction_spec`...) cho ràng buộc giá trị; request spec (`zones_spec`, `units_spec`, `contact_points_spec`) cho ràng buộc xóa cascade.
