...
**Nếu bạn là Chỉ huy Sư đoàn:**

Bạn xem toàn bộ hệ thống ở chế độ chỉ xem — tương tự quản trị viên hệ thống nhưng không tạo/sửa/xóa được gì. Bạn có thể bấm **Tính toán lại** trên trang Bảng tính tiền. Bạn cần đọc:

- Mục B (đăng nhập, đổi mật khẩu).
- Mục E1 (tổng quan toàn hệ thống — xem tình hình chung mọi đơn vị).
- Mục E2 (bảng tính tiền — xem chi tiết, lọc theo khu vực/đơn vị, xuất Excel).
- Mục E3 (tra cứu lịch sử — so sánh giữa các kỳ).

Bạn không cần đọc các mục khai báo/cấu hình (F) hay quản trị hệ thống (G) vì không thao tác được.
...
Nếu đơn vị bạn là đơn vị quản lý khu vực, bạn thấy thêm 3 trang về khu vực (Nhập số điện lực, Chỉ số bơm nước, Phân bổ bơm nước) — tất cả ở chế độ chỉ xem.
...
Vào trang **Đơn giá điện** (xem mục F7), nhập đơn giá điện, chọn tháng/năm, bấm **Mở kỳ mới**. Từ bước này trở đi mọi thay đổi dữ liệu nghiệp vụ đều cần kỳ đang mở.

**Bước 4 — Quản trị viên hệ thống: Tạo khu vực.**
Vào trang **Khu vực** (xem mục F4), bấm **Tạo mới**. Nhập tên khu vực và tên công tơ tổng. Lặp lại cho tất cả khu vực cần thiết.
...
Vào trang **Tài khoản** (xem mục G1), tạo tài khoản cho Chỉ huy Sư đoàn (nếu có), quản trị viên đơn vị và chỉ huy đơn vị. Mỗi tài khoản cần: tên đăng nhập, mật khẩu, tên hiển thị, vai trò, đơn vị (bắt buộc cho quản trị viên đơn vị và chỉ huy đơn vị; Chỉ huy Sư đoàn không cần đơn vị).
...
- Vào trang **Đầu mối** (xem mục F1), tạo đầu mối sinh hoạt: nhập tên, nhập quân số theo 7 nhóm cấp bậc (tổng quân số phải ≥ 1), thêm ít nhất 1 công tơ (tên, có thể đánh dấu "không tổn hao").
- Tạo đầu mối công cộng: nhập tên, thêm ít nhất 1 công tơ.
- Vào trang **Khối** và **Nhóm** (xem mục F2) nếu cần gộp đầu mối để hiển thị trên bảng tính tiền.
- Vào trang **Cấu hình đơn vị** (xem mục F3) để thiết lập tỷ lệ công cộng dùng chung đơn vị và giá trị cột Khác cho từng đầu mối.
...
- Nếu quân số có thay đổi: vào trang **Đầu mối** (xem mục F1), bấm sửa đầu mối sinh hoạt cần cập nhật, sửa quân số theo nhóm cấp bậc.
- Nếu cấu hình đơn vị cần cập nhật: vào trang **Cấu hình đơn vị** (xem mục F3), sửa tỷ lệ hoặc giá trị cột Khác.
...
- Chỉ huy Sư đoàn: xem tổng quan và bảng tính tiền toàn hệ thống, chế độ chỉ xem (xem mục E1, E2, E3).
...
- **Tổn hao:** Số kW tổn hao phân bổ cho công tơ đó. Chỉ đọc — hệ thống tự điền sau khi bấm "Tính toán lại" trên trang Bảng tính tiền. Để trống khi chưa tính.
- **Sử dụng thực tế:** = Sử dụng + Tổn hao. Chỉ đọc, để trống khi chưa tính.
- Bấm **Lưu** để lưu toàn bộ trang 1 lần (không cần lưu từng dòng).

Hai cột Tổn hao và Sử dụng thực tế là kết quả lần tính gần nhất — có thể chưa cập nhật nếu vừa sửa chỉ số. Bấm "Tính toán lại" ở Bảng tính tiền để làm mới.
...
| Chỉ huy Sư đoàn | Tất cả (có ô lọc khu vực và đơn vị, có cột Khu vực và Đơn vị) | Không (chỉ xem) |
...
Trang **Chỉ số bơm nước** dùng để nhập chỉ số công tơ bơm nước (trạm bơm). Giao diện giống trang Chỉ số đầu mối — bảng, tìm kiếm, lưu toàn bộ 1 lần. Cũng có 2 cột chỉ đọc Tổn hao và Sử dụng thực tế (xem D1).
...
| Chỉ huy Sư đoàn | Tất cả (có ô lọc khu vực, có cột Khu vực) | Không (chỉ xem) |
...
**Quản trị viên hệ thống** và **Chỉ huy Sư đoàn** thấy tổng quan hệ thống: tổng thâm điện và thành tiền theo từng đơn vị (sắp xếp thâm điện từ nhiều đến ít), tổng sử dụng điện công cộng và điện bơm nước toàn khu vực, trạng thái nhập liệu từng đơn vị ("chưa nhập" hoặc "đã nhập"), cảnh báo dữ liệu thiếu và cảnh báo tổn hao bất thường.

![Tổng quan — Quản trị viên hệ thống](images/02_tong_quan_sa.png)

Khi có khu vực cần tính toán lại (dữ liệu đã đổi sau lần tính gần nhất), trang hiện **khung cảnh báo vàng** liệt kê từng khu vực cần cập nhật, kèm hướng dẫn vào Bảng tính tiền bấm "Tính toán lại".
...
**Chỉ báo độ tươi dữ liệu:** Khi có khu vực mà dữ liệu đã đổi sau lần tính gần nhất, trang hiện **khung cảnh báo vàng** liệt kê từng khu vực cần tính lại. Bấm "Tính toán lại" để cập nhật — sau đó cảnh báo biến mất.

**Đối chiếu tổn hao:** Bên dưới bảng tính tiền, mỗi khu vực có phần **"Đối chiếu"** (bấm để gập/mở) chứa bảng đối chiếu sử dụng và tổn hao theo loại đầu mối:

| Cột | Ý nghĩa |
|---|---|
| Loại | Sinh hoạt, Công cộng, Bơm nước |
| Sử dụng | Tổng sử dụng theo loại |
| Tổn hao | Tổng tổn hao theo loại |
| Sử dụng thực tế | = Sử dụng + Tổn hao |

Bảng gồm dòng "Cộng (công tơ có tổn hao)" kèm nhãn A/B/C, dòng "Không tổn hao", và dòng "Tổng cộng (= số công tơ tổng)". Đúng khi A = B + C; lệch do làm tròn tối đa 0,01. Phần này chỉ đọc, giúp kiểm tra kết quả tính toán.

**Đối chiếu điện bơm nước theo trạm:** Nếu kỳ hiện tại dùng chế độ phân bổ theo trạm (xem mục F6), phần đối chiếu còn có bảng phân bổ điện bơm nước: mỗi hàng là một đối tượng nhận, mỗi cột là một trạm bơm (kèm phần trăm so với khu vực), giúp kiểm tra điện đã chia đúng chưa.
...
Quản trị viên đơn vị và chỉ huy đơn vị chỉ xem lịch sử đơn vị mình. Quản trị viên hệ thống và Chỉ huy Sư đoàn xem tất cả.

### E4. Xuất Excel
Trên trang Bảng tính tiền, bấm nút **Xuất Excel**. File Excel tải về máy, nội dung giống hệt bảng trên hệ thống. File có đầy đủ công thức tính toán (không chỉ giá trị tĩnh) — ví dụ: ô tổng tiêu chuẩn chứa công thức = tiêu chuẩn điện sinh hoạt + tiêu chuẩn điện bơm nước. Cuối sheet có bảng đối chiếu tổn hao A/B/C (xem mục E2). Người dùng có thể mở file Excel để xử lý thêm nếu cần.

**Cảnh báo dữ liệu cũ:** Nếu có khu vực cần tính toán lại (dữ liệu đã đổi sau lần tính gần nhất), khi bấm "Xuất Excel" hệ thống hiện hộp xác nhận: "Dữ liệu một số khu vực đã cũ so với lần tính gần nhất. Vẫn xuất Excel?". Nếu chọn "Vẫn xuất", file tải về nhưng chứa dòng cảnh báo: "CẢNH BÁO: Dữ liệu trong file có thể đã cũ so với chỉ số hiện tại. Hãy Tính toán lại để cập nhật." Nên bấm "Tính toán lại" trước khi xuất để có số liệu chính xác.
...
![Tạo đầu mối sinh hoạt](images/09_dau_moi_tao_sinh_hoat.png)

- **Công cộng:** Tên, thuộc đơn vị hoặc khu vực, ít nhất 1 công tơ.

![Tạo đầu mối công cộng](images/10_dau_moi_tao_cong_cong.png)

- **Bơm nước:** Tên, khu vực, ít nhất 1 công tơ.

![Tạo đầu mối bơm nước](images/11_dau_moi_tao_bom_nuoc.png)

- **Ngoài biên chế:** Tên, khu vực, quân số (≥ 1, 1 con số tổng).

![Tạo đầu mối ngoài biên chế](images/12_dau_moi_tao_ngoai_bien_che.png)

Đầu mối sinh hoạt và công cộng có thể thuộc đơn vị hoặc khu vực. Quản trị viên hệ thống và quản trị viên đơn vị quản lý khu vực thấy ô chọn "Thuộc đơn vị" hoặc "Thuộc khu vực" trên biểu mẫu (xem hình dưới). Quản trị viên đơn vị (không quản lý khu vực) tạo đầu mối thì mặc định thuộc đơn vị mình, không cần chọn. Chỉ huy Sư đoàn xem trang này ở chế độ chỉ xem (thấy tất cả đầu mối toàn hệ thống, không tạo/sửa/xóa được).
...
Trang **Cấu hình đơn vị** cho phép thiết lập cấu hình công cộng và khoản trừ của đơn vị. Quản trị viên hệ thống sửa được mọi đơn vị. Quản trị viên đơn vị sửa đơn vị mình (nếu là đơn vị quản lý khu vực thì thêm cột Khác cho đầu mối sinh hoạt thuộc khu vực). Chỉ huy Sư đoàn và chỉ huy đơn vị xem ở chế độ chỉ đọc. Các thông tin thiết lập:
...
- **Cột Khác** từng đầu mối sinh hoạt: giá trị bổ sung trừ hoặc cộng vào tiêu chuẩn. Có 3 dạng nhập:
  - **Cố định:** Nhập trực tiếp giá trị kW. Cho phép âm (âm = cộng ngược vào tiêu chuẩn).
  - **Theo hệ số:** Nhập hệ số, hệ thống tự tính = hệ số × quân số đầu mối. Ví dụ: nhập hệ số 5, đầu mối có 10 người → khoản trừ = 5 × 10 = 50 kW.
  - **Theo hệ số (đơn vị):** Nhập hệ số, hệ thống tự tính = hệ số × (tổng quân số đơn vị − quân số đầu mối). Dùng cho trường hợp bếp ăn chung: mỗi người trong đơn vị góp một phần, bếp nhận lại tổng. Ví dụ: hệ số −2, đơn vị có 74 người, bếp có 8 người → khoản trừ = −2 × (74 − 8) = −132 kW (âm = cộng ngược vào tiêu chuẩn). Giá trị tự tính lại khi quân số thay đổi. Lựa chọn này chỉ hiện cho đầu mối thuộc đơn vị — đầu mối thuộc khu vực trực tiếp không có.
...
Trang **Khu vực** quản lý khu vực và công tơ tổng. Chỉ quản trị viên hệ thống được thao tác (tạo, sửa, xóa khu vực). Chỉ huy Sư đoàn xem tất cả khu vực ở chế độ chỉ đọc.
...
![Tạo đơn vị](images/76_don_vi_tao_moi.png)
...
Trang **Phân bổ bơm nước** cấu hình cách phân bổ điện bơm nước cho các đối tượng trong khu vực. Quản trị viên hệ thống thao tác trên tất cả khu vực. Quản trị viên đơn vị quản lý khu vực thao tác trên khu vực mình. Chỉ huy Sư đoàn và chỉ huy đơn vị quản lý khu vực xem ở chế độ chỉ đọc.

**Hai chế độ phân bổ** (xác định theo từng kỳ):

- **Theo từng trạm bơm (mới):** Mỗi trạm bơm (đầu mối bơm nước) có bảng đối tượng nhận riêng. Trang hiển thị **một thẻ cho mỗi trạm** — tiêu đề ghi tên trạm, số kW, và phần trăm so với khu vực (ví dụ: "Trạm bơm Tây — 150,00 kW (chiếm 65% điện bơm của khu vực)"). Bên trong mỗi thẻ là bảng đối tượng nhận với cột % cố định và Hệ số. Bấm "Thêm đối tượng vào trạm này" để thêm.
- **Gộp toàn khu vực (kỳ cũ):** Tổng điện bơm nước toàn khu vực gộp lại rồi chia — hiện một thẻ duy nhất "Gộp toàn khu vực (kỳ cũ)". Đây là chế độ của các kỳ trước khi bật phân bổ theo trạm.

**Bốn loại đối tượng nhận:** Đơn vị, Khối, Nhóm, hoặc Đầu mối (sinh hoạt hoặc ngoài biên chế). Khi thêm đối tượng, chọn loại rồi chọn cụ thể từ danh sách.

**Hai cách phân bổ** cho mỗi đối tượng (giống nhau ở cả 2 chế độ):

- **Phần trăm cố định:** Đối tượng nhận = tổng điện trạm × phần trăm. Tổng phần trăm cố định trong cùng trạm không được vượt 100%.
...
![Phân bổ bơm nước — cột "% Cố định" và cột "Hệ số" thể hiện 2 cách phân bổ](images/21_phan_bo_bom_nuoc_sa.png)

**Ví dụ (chế độ theo trạm):** Trạm bơm Tây có 3.900 kW (chiếm 65% khu vực). Cấu hình: đầu mối "Chỉ huy" nhận 30% cố định, Đơn vị A (200 người, hệ số 1) và Đơn vị B (150 người, hệ số 1) nhận phần còn lại.

- Chỉ huy nhận: 3.900 × 30% = 1.170 kW
- Phần còn lại: 3.900 − 1.170 = 2.730 kW
- Đơn vị A nhận: 2.730 × 200 ÷ (200 + 150) = 1.560 kW
- Đơn vị B nhận: 2.730 × 150 ÷ (200 + 150) = 1.170 kW
...
**Kế thừa khi mở kỳ mới:** Kỳ theo trạm kế thừa cấu hình từ kỳ trước (nếu kỳ trước cũng theo trạm). **Lưu ý:** kỳ đầu tiên chuyển từ gộp sang theo trạm bắt đầu trống — phải cấu hình lại từ đầu vì cấu trúc phân bổ khác nhau hoàn toàn.

### F7. Đơn giá điện và kỳ tính toán

Trang **Đơn giá điện** quản lý đơn giá, cấu hình chung, và kỳ tính toán. Chỉ quản trị viên hệ thống được thao tác. Chỉ huy Sư đoàn xem ở chế độ chỉ đọc.
...
Trang **Nhóm cấp bậc** quản lý các nhóm cấp bậc và định mức tương ứng. Chỉ quản trị viên hệ thống được thao tác. Chỉ huy Sư đoàn xem ở chế độ chỉ đọc.
...
- **Quản trị viên hệ thống** quản lý tài khoản quản trị viên hệ thống, Chỉ huy Sư đoàn, quản trị viên đơn vị, và chỉ huy đơn vị (không quản lý tài khoản kỹ thuật viên).
...
Trang **Nhật ký hoạt động** (quản trị viên hệ thống, Chỉ huy Sư đoàn, và kỹ thuật viên) hiển thị lịch sử mọi thao tác trên hệ thống: ai thay đổi, thay đổi gì, lúc nào. Có thể lọc theo loại thao tác (tạo/sửa/xóa), đối tượng, người thao tác, và khoảng thời gian.
...
| Cột Khác | Khoản trừ (hoặc cộng) bổ sung cho đầu mối sinh hoạt, có 3 dạng: cố định, theo hệ số, theo hệ số (đơn vị) |
| Phân bổ bơm nước | Cách chia điện bơm nước cho các đối tượng (theo % cố định hoặc hệ số × quân số). Có 2 chế độ: theo từng trạm bơm hoặc gộp toàn khu vực |
| Chỉ báo độ tươi | Khung cảnh báo vàng trên các trang khi dữ liệu đã đổi sau lần tính gần nhất — nhắc bấm "Tính toán lại" |
...
- Mục A5 + F6: bỏ "công cộng" khỏi danh sách đối tượng nhận phân bổ bơm nước — đầu mối công cộng không có quân số nên không phải đối tượng nhận hợp lệ (khớp V2_XAC_NHAN_NGHIEP_VU mục 9.2 và ADR-026). Issue #459.
...
- Mục F6: sửa "Đầu mối sinh hoạt" → "Đầu mối (sinh hoạt, công cộng, hoặc ngoài biên chế — không bao gồm bơm nước)" cho khớp form thực tế (dropdown chứa 3 loại contact_point_type).
...
- Cập nhật toàn bộ 3 tính năng milestone 1.2.0 + chỉ báo độ tươi (Issue #428):
  - Mục A5: mở rộng mô tả phân bổ bơm nước (thêm loại đối tượng nhận, chế độ theo trạm).
  - Mục D bước 4: thêm mô tả chỉ báo độ tươi (khung vàng khi cần tính lại).
  - Mục D1: thêm 2 cột chỉ đọc Tổn hao và Sử dụng thực tế, ghi chú giá trị snapshot.
  - Mục D2: thêm tham chiếu 2 cột mới (xem D1).
  - Mục E1: thêm chỉ báo độ tươi per khu vực trên trang Tổng quan.
  - Mục E2: thêm chỉ báo độ tươi, phần đối chiếu tổn hao A/B/C, đối chiếu điện bơm theo trạm.
  - Mục E4: thêm bảng A/B/C trong Excel, cảnh báo dữ liệu cũ (hộp xác nhận + dòng đóng dấu trong file).
  - Mục F3: "2 dạng nhập" → "3 dạng nhập", thêm "Theo hệ số (đơn vị)" với ví dụ bếp ăn chung.
  - Mục F6: viết lại — 2 chế độ phân bổ (theo trạm / gộp khu vực), 4 loại đối tượng nhận, thẻ per trạm, ràng buộc, kế thừa kỳ (kỳ đầu per-trạm trống).
  - Mục H1: thêm thuật ngữ "Cột Khác" (3 dạng), "Chỉ báo độ tươi"; cập nhật "Phân bổ bơm nước" (2 chế độ).

### v1.8.1 (22/06/2026)

- Mục B5 chỉ huy đơn vị: sửa "thêm trang khu vực" → bỏ Khu vực (zone-manager đã bị chặn khỏi /zones từ SettingsAccessGuard `require_system_admin!`). Khớp mục B4 (3 mục, không 4).
- Mục G1: thêm Chỉ huy Sư đoàn vào danh sách vai trò mà quản trị viên hệ thống quản lý được.

### v1.8.0 (22/06/2026)

- Thêm vai trò Chỉ huy Sư đoàn xuyên suốt toàn bộ tài liệu (Issue #419):
  - Header: thêm vào danh sách đối tượng.
  - Mục A6: đổi "Bốn vai trò" → "Năm vai trò", thêm hàng bảng + đoạn mô tả chi tiết.
  - Mục B4: thêm vào danh sách sidebar (16 mục). Sửa đơn vị quản lý khu vực "thêm 4 mục" → "thêm 3 mục".
  - Mục B5: thêm mục "Nếu bạn là Chỉ huy Sư đoàn" (hướng dẫn bắt đầu nhanh).
  - Mục C bước 7: thêm tạo tài khoản Chỉ huy Sư đoàn.
  - Mục D bước 5: thêm Chỉ huy Sư đoàn vào danh sách kiểm tra kết quả.
  - Mục D1, D2: thêm hàng Chỉ huy Sư đoàn vào bảng "Ai thấy gì".
  - Mục E1: quản trị viên hệ thống → "quản trị viên hệ thống và Chỉ huy Sư đoàn".
  - Mục E2: thêm ghi chú Chỉ huy Sư đoàn xem toàn hệ thống + có nút Tính toán lại.
  - Mục E3: thêm Chỉ huy Sư đoàn xem tất cả lịch sử.
  - Mục F1: thêm ghi chú Chỉ huy Sư đoàn xem chỉ đọc toàn hệ thống.
  - Mục F3: thêm Chỉ huy Sư đoàn vào danh sách xem chỉ đọc.
  - Mục F4: thêm Chỉ huy Sư đoàn xem chỉ đọc. Bỏ ghi chú cũ về zone-manager xem trang này (zone-manager đã bị chặn từ v2.0.3 V2_KICH_BAN_TEST).
  - Mục F5, F7, F8: sửa "(chỉ quản trị viên hệ thống)" → thêm Chỉ huy Sư đoàn xem chỉ đọc.
  - Mục F6: thêm Chỉ huy Sư đoàn xem chỉ đọc.
  - Mục G1: đổi "4 vai trò" → "5 vai trò", thêm vào danh sách tạo tài khoản.
  - Mục G2: thêm Chỉ huy Sư đoàn vào danh sách vai trò thấy nhật ký.
