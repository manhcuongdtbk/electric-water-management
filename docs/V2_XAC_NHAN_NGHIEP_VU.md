...
**Ràng buộc mỗi trạm:** mỗi trạm phải có ít nhất một đối tượng nhận (trạm chưa cấu hình → cảnh báo); tổng phần trăm cố định ≤ 100% theo từng trạm; phải có đối tượng nhận hệ số nếu chưa đạt 100%; tổng (quân số × hệ số) > 0. Đối tượng nhận kiểu đơn vị, khối, hoặc nhóm phải có ít nhất 1 đầu mối sinh hoạt bên trong (không cho phép đối tượng rỗng — tránh chia cho 0 khi phân phối).

**Ràng buộc xuyên trạm (toàn khu vực):**

- **Không chồng chéo:** tập đầu mối sinh hoạt mà mỗi đối tượng nhận "phân giải" tới phải không giao nhau — cả trong cùng trạm lẫn giữa các trạm. Ví dụ: nếu đơn vị đã nhận từ một trạm, khối hoặc đầu mối bên trong đơn vị đó không được nhận từ bất kỳ trạm nào (kể cả cùng trạm). Và ngược lại. Ràng buộc này đảm bảo mỗi đầu mối sinh hoạt nhận điện bơm nước từ đúng 1 nguồn, không tính trùng.

Các khối, nhóm, đầu mối trong cùng đơn vị **có thể** nằm ở các trạm bơm khác nhau — trạm bơm phục vụ vùng vật lý, không phụ thuộc cấp tổ chức (ADR-067).
...
- **Kỳ phân bổ-theo-trạm đầu tiên** (ngay sau khi cập nhật hệ thống): bắt đầu **trống**; quản trị viên cấu hình lại từng trạm. Lý do không kế thừa: (1) cấu hình gộp cũ không gắn được vào trạm cụ thể; (2) phần trăm cố định đổi ngữ nghĩa — kỳ gộp là phần trăm của **toàn khu vực**, kỳ theo-trạm là phần trăm của **trạm đó** — kế thừa con số cũ sẽ sai ý nghĩa.
...
- Mục 9.6 ràng buộc xuyên trạm: bỏ ràng buộc "không chia cấp" (ADR-067, Issue #481). Trạm bơm phục vụ vùng vật lý, không phụ thuộc cấp tổ chức — các khối/nhóm/đầu mối trong cùng đơn vị có thể nằm ở các trạm bơm khác nhau. Giữ nguyên "không chồng chéo" (mỗi đầu mối sinh hoạt nhận từ đúng 1 nguồn). Phát hiện qua Acceptance v1.2.0 với khách.
