...
- **Snapshot mỗi kỳ.** Dữ liệu thay đổi giữa các kỳ được copy khi mở kỳ mới. Engine tính toán dùng snapshot, không dùng giá trị hiện tại.
...
| meter_readings | (meter_id, period_id) | Unique | Mỗi công tơ chỉ có 1 reading mỗi kỳ |
| main_meter_readings | (main_meter_id, period_id) | Unique | Mỗi công tơ tổng chỉ có 1 reading mỗi kỳ |
...
Bảng riêng phòng tương lai nhiều công tơ tổng mỗi khu vực. Hiện tại mỗi khu vực có đúng 1: luồng tạo/sửa khu vực chỉ cho tạo đúng 1 công tơ tổng. Chưa đặt unique constraint trên main_meters.zone_id ở database vì bảng để mở cho tương lai nhiều công tơ tổng.
...
#### ranks (nhóm cấp bậc — snapshot mỗi kỳ)
...
#### meter_readings (chỉ số công tơ mỗi kỳ)
...
#### main_meter_readings (chỉ số công tơ tổng mỗi kỳ)
...
#### loss_summaries (tóm tắt tổn hao A/B/C mỗi khu vực mỗi kỳ)
...
#### personnel_entries (quân số mỗi đầu mối sinh hoạt mỗi nhóm cấp bậc mỗi kỳ)
...
#### non_establishment_snapshots (quân số đầu mối ngoài biên chế mỗi kỳ)
...
#### unit_configs (cấu hình mỗi đơn vị mỗi kỳ)
...
#### other_deductions (khoản trừ "Khác" mỗi đầu mối mỗi kỳ)
...
#### pump_allocations (phân bổ bơm nước — snapshot mỗi kỳ)
...
#### calculations (kết quả tính toán mỗi đầu mối sinh hoạt mỗi kỳ)
...
#### pump_station_charges (phân bổ bơm nước mỗi trạm mỗi đầu mối — ADR-054)
...
Snapshot kết quả phân bổ mỗi trạm, ghi bởi `PumpStationChargeWriter` trong transaction orchestrator. Dùng cho bảng đối chiếu điện bơm nước mỗi trạm trên trang Bảng tính tiền. Không kế thừa kỳ.
...
CalculationOrchestrator
  ...
  └── SummaryCalculator     (tổng hợp — tính mỗi đầu mối sinh hoạt)
```
...
- `no_loss` lấy từ `meter_readings.no_loss` (snapshot mỗi kỳ), không phải `meters.no_loss`.
- Quân số đầu mối sinh hoạt lấy từ `personnel_entries` (mỗi kỳ).
- Quân số đầu mối ngoài biên chế lấy từ `non_establishment_snapshots` (mỗi kỳ).
- Định mức cấp bậc lấy từ `ranks` (mỗi kỳ).
...
- Engine dùng `.with_discarded` khi query zones, units, contact_points, meters, main_meters — để tính toán lại kỳ cũ vẫn thấy thực thể đã xóa (soft delete). Dữ liệu mỗi kỳ (meter_readings, personnel_entries, calculations...) dùng period_id nên luôn tìm đúng, không cần `.with_discarded`.
...
Output: hash chứa tổn hao mỗi công tơ, tổn hao mỗi đầu mối, tổng tổn hao khu vực, warnings.
...
Phân bổ cho các đối tượng trong `pump_allocations` (snapshot mỗi kỳ):
...
Flow:
1. LossCalculator.new(zone:, period:).call → loss_results (gồm A/B/C + tổn hao mỗi công tơ)
...
| Dữ liệu | Nguồn | Đích |
|---|---|---|
| Số đầu kỳ công tơ | meter_readings.reading_end kỳ trước (editable — user có thể sửa nếu cần) | meter_readings.reading_start kỳ mới |
...
**Cho phép sửa (nhập liệu mỗi kỳ):** meter_readings, main_meter_readings, personnel_entries, non_establishment_snapshots, unit_configs, other_deductions, pump_allocations.
...
> **Lý do (v2.3.0):** Thay đổi cấu trúc (tạo/xóa zone, đơn vị, đầu mối...) ảnh hưởng mọi kỳ vì các thực thể cấu trúc không có snapshot mỗi kỳ. Nếu cho phép thay đổi cấu trúc khi mở kỳ cũ → vi phạm nguyên tắc "mọi thao tác ở kỳ đang mở không ảnh hưởng kỳ đã đóng" (kỳ mới hơn đã đóng sẽ bị ảnh hưởng). Nghiệp vụ quy định mở kỳ cũ chỉ để sửa sai sót nhập liệu — không cần thay đổi cấu trúc.
...
| Bảng tính tiền | /billing | billing#show | Bảng tính tiền mỗi đơn vị/khu vực + xuất Excel |
| Tra cứu lịch sử | /history | history#show | Tra cứu + so sánh 2 kỳ |

**NHẬP LIỆU HÀNG THÁNG:**

Tất cả trang nhập liệu dùng chung UI pattern: form kiểu bảng — tất cả đầu mối trên 1 trang, mỗi hàng = 1 đầu mối, cột = ô nhập. Save toàn bộ trang 1 lần (không save từng hàng). Hiển thị flash thành công/lỗi sau khi save. Trang chỉ số đầu mối và chỉ số bơm nước có tìm kiếm theo tên đầu mối, filter khu vực/đơn vị (SA only), cột khu vực/đơn vị (SA only). Số đầu kỳ (reading_start) editable mọi kỳ — kế thừa từ kỳ trước nhưng cho phép sửa.

| Tên sidebar | Route | Controller | Chức năng |
|---|---|---|---|
| Nhập số điện lực | /electricity_supply | electricity_supply#show | Nhập số sử dụng công tơ tổng (main_meter_readings) |
| Chỉ số đầu mối | /meter_entries | meter_entries#show | Nhập chỉ số công tơ đầu mối sinh hoạt + công cộng. SA có search + filter zone/unit + cột zone/unit. Unit_admin zone-manager thấy thêm đầu mối sinh hoạt + công cộng thuộc khu vực |
| Chỉ số bơm nước | /pump_entries | pump_entries#show | Nhập chỉ số công tơ bơm nước. SA có search + filter zone + cột zone (không có unit — bơm nước luôn thuộc khu vực) |
...
| Cấu hình đơn vị | /unit_config | unit_config#show | Công cộng đơn vị, cột Khác theo đầu mối |
...
| Phân bổ bơm nước | /pump_allocations | pump_allocations | Cấu hình phân bổ theo khu vực |
| Đơn giá điện | /pricing | pricing#show | Đơn giá mỗi kỳ + mở/đóng kỳ |
...
Chọn kỳ bắt đầu và kỳ kết thúc, hệ thống liệt kê các kỳ trong khoảng đó kèm tổng thâm điện và thành tiền.
...
Lưu ý: bảng tính tiền cần hiển thị 7 cột quân số mỗi nhóm cấp bậc. Dữ liệu này lấy từ bảng `personnel_entries` (có period_id), không phải từ bảng `calculations` (chỉ lưu total_personnel). Khi xem kỳ cũ, personnel_entries kỳ đó vẫn còn nguyên.
...
Khi discard thực thể cấu trúc, nếu có kỳ đang mở → hard delete data mỗi kỳ đang mở của thực thể đó. Kỳ cũ giữ nguyên. Nếu không có kỳ đang mở → không xóa gì.

> **Lý do:** Engine dùng `.with_discarded` để thấy thực thể đã xóa khi tính toán lại kỳ cũ. Nhưng nếu data mỗi kỳ đang mở vẫn còn, engine sẽ cảnh báo/tính toán sai cho thực thể đã xóa. Xóa data mỗi kỳ đang mở → engine kiểm tra có meter_readings kỳ đó không → không có → skip.
...
- Đơn giá điện hiển thị đầy đủ không làm tròn, không cắt thập phân (ví dụ: 2.336,4 đ/kW). Áp dụng trên: trang đơn giá điện, bảng tính tiền, tổng quan, tra cứu lịch sử.
...
- `meter_readings`: thêm cột `loss` (decimal, nullable) — snapshot tổn hao mỗi công tơ, ghi trong transaction `CalculationOrchestrator`. "Sử dụng thực tế" = sử dụng + loss (tính khi render).
- Bảng mới `loss_summaries` (`zone_id`, `period_id`, `a`, `b`, `c`, unique `(zone_id, period_id)`) — tóm tắt A/B/C mỗi khu vực mỗi kỳ.
...
- Thay jargon tiếng Anh "per" bằng "mỗi"/"theo" trong toàn bộ text tiếng Việt (ngoài code fence, backtick, changelog). Issue #432.

### v2.17.3 (25/06/2026)

- Sửa 2 drift sinh ra từ cross-check audit session (#456/#460 sửa V2_XAC_NHAN + V2_CHIEU_TEST nhưng quên V2_THIET_KE):
  - Sidebar routes: pump_entries sửa "filter zone/unit + cột zone/unit" → "filter zone + cột zone (không có unit — bơm nước luôn thuộc khu vực)" — khớp V2_CHIEU_TEST v1.5.3 và code.
  - History range: sửa "tháng, quý, năm, tuỳ chọn ngày" → "chọn kỳ bắt đầu và kỳ kết thúc" — khớp V2_XAC_NHAN v2.19.2 và code.
...
- Nhập liệu: meter_entries và pump_entries thêm tìm kiếm theo tên đầu mối, filter khu vực/đơn vị (SA only), cột khu vực/đơn vị (SA only).
- Nhập liệu: reading_start editable mọi kỳ (không chỉ kỳ đầu tiên). Kế thừa từ kỳ trước nhưng cho phép sửa.
- Hiển thị số: đơn giá điện hiển thị đầy đủ không làm tròn trên 4 trang (đơn giá điện, bảng tính tiền, tổng quan, tra cứu lịch sử).
