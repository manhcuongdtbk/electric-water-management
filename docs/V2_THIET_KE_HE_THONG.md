...
Dùng gem **discard**. Các bảng có soft delete: contact_points, meters, units, blocks, groups, main_meters. Thêm cột `discarded_at` (datetime, nullable, indexed).
...
Zones không soft delete — nghiệp vụ yêu cầu xóa hết đơn vị trước khi xóa khu vực.
