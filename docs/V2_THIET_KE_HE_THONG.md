...
- Engine phải dùng `.with_discarded` khi query zones, units, contact_points, meters, main_meters — để tính toán lại kỳ cũ vẫn thấy thực thể đã xóa (soft delete). Dữ liệu per kỳ (meter_readings, personnel_entries, calculations...) dùng period_id nên luôn tìm đúng, không cần `.with_discarded`.
...
| meter_readings (công tơ thuộc đơn vị mình) | CRUD |
| personnel_entries (đầu mối thuộc đơn vị mình) | CRUD |
| unit_configs (đơn vị mình) | CRUD |
| other_deductions (đầu mối thuộc đơn vị mình) | CRUD |
...
| meter_readings công tơ bơm nước (khu vực mình quản lý) | CRUD |
| meter_readings đầu mối sinh hoạt thuộc khu vực | CRUD |
| meter_readings đầu mối công cộng thuộc khu vực | CRUD |
| personnel_entries đầu mối sinh hoạt thuộc khu vực | CRUD |
| non_establishment_snapshots (khu vực mình quản lý) | CRUD |
| Bảng tính tiền | Xem đơn vị mình + đầu mối sinh hoạt thuộc khu vực |
| pump_allocations | Không (chỉ system_admin quản lý cấu hình phân bổ) |
...
| Nếu đơn vị quản lý khu vực | Xem bảng tính tiền bao gồm đầu mối sinh hoạt thuộc khu vực |
...
| Dữ liệu nghiệp vụ | Không truy cập |
...
Sidebar item luôn hiển thị (không ẩn). Khi không có kỳ đang mở: các trang nhập liệu vẫn truy cập được nhưng hiển thị thông báo "Không có kỳ đang mở" và disable mọi ô nhập (PeriodGuard). Các trang xem kết quả và khai báo hoạt động bình thường.
...
8. **Quản trị viên hệ thống: Cấu hình khu vực.** Đầu mối bơm nước + công tơ. Đầu mối sinh hoạt thuộc khu vực. Đầu mối công cộng thuộc khu vực. Đầu mối ngoài biên chế. Phân bổ bơm nước. Điều chỉnh cấu hình chung nếu cần.
...
3. **Đơn vị quản lý khu vực: Nhập liệu phần khu vực.** Nhập số sử dụng công tơ tổng. Nhập số cuối kỳ công tơ bơm nước. Nhập số cuối kỳ công tơ đầu mối thuộc khu vực.
