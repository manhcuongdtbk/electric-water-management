...
## Các loại input cần kiểm thử

Mỗi trang có tập input riêng. Khi test phải cover tất cả loại input, không chỉ output.

### R (Read/List) — trang danh sách

5 loại input chung cho mọi trang danh sách (nghiệp vụ mục 19):

| Input | Param | Cần kiểm thử |
|---|---|---|
| Tìm kiếm | `q` | Text bình thường, ký tự đặc biệt ILIKE (`%`, `_`), chuỗi rỗng, khoảng trắng đầu cuối |
| Sắp xếp | `sort`, `dir` | Mỗi cột sortable, chiều ASC/DESC, giá trị sort không hợp lệ |
| Lọc (filter) | Tùy trang (`zone_id`, `unit_id`, `type`...) | Chọn filter, đổi filter, xóa filter, kết hợp filter + search |
| Số dòng mỗi trang | `per_page` | Các giá trị hợp lệ (10, 25, 50, 100), giá trị ngoài danh sách |
| Phân trang | `page` | Trang đầu, trang cuối, trang ngoài phạm vi |

Input đặc thù per trang:

| Trang | Input đặc thù |
|---|---|
| Bảng tính tiền (/billing) | Chọn kỳ (`period_id`), dropdown zone/unit (SA only) |
| Tra cứu lịch sử (/history) | Chọn kỳ A + kỳ B (compare), chọn khoảng thời gian (range), chọn mode |
...
|---|---|---|
| Field bắt buộc | name, contact_point_type, zone_id/unit_id | Trống → lỗi validation, có giá trị → thành công |
| Field tùy chọn | block_id, group_id, no_loss | Bỏ trống → giá trị mặc định, có giá trị → lưu đúng |
| Nested attributes | meters trong contact_point (tên + no_loss) | Tạo kèm ≥ 1 meter, không có meter → chặn |
| Personnel counts per rank | Quân số theo từng nhóm cấp bậc (chỉ residential) | Tổng ≥ 1, từng rank ≥ 0 |
| Giá trị biên | 0, 100%, giá trị âm (other_deduction), giá trị rất lớn | Validation đúng theo ràng buộc mục 24 nghiệp vụ |
| Tên trùng | Trùng tên trong cùng phạm vi (unit, zone, loại CP) | Chặn đúng scope uniqueness |

### U (Update) — cập nhật

| Loại input | Ví dụ | Cần kiểm thử |
|---|---|---|
| Field immutable | contact_point_type, unit.zone_id | Submit giá trị mới → bị bỏ qua, không đổi |
| Field mutable | name, block_id, group_id, no_loss | Đổi giá trị → lưu đúng |
| lock_version | Optimistic locking | 2 user sửa cùng lúc → người sau nhận cảnh báo "dữ liệu đã bị thay đổi" |
| Batch update | meter_entries, pump_entries, electricity_supply, unit_config | Sửa N records trên 1 trang, submit 1 lần. Lỗi 1 record → rollback tất cả |
| Kỳ cũ mở lại | Sửa data per kỳ khi StructureChangeGuard active | Sửa reading_end, quân số → được. Sửa tên, tạo/xóa entity → chặn |

### D (Delete) — xóa

| Loại input | Ví dụ | Cần kiểm thử |
|---|---|---|
| Validation chặn xóa | Công tơ cuối cùng, đơn vị có đầu mối, khu vực có đơn vị, rank đang dùng, tài khoản mặc định, tự xóa mình | Mỗi trường hợp → thông báo lỗi đúng |
| Cascade | Xóa khối → nhóm/CP mất block_id. Xóa nhóm → CP mất group_id (lên cấp trên) | Cascade đúng, CP không mất data |
| Cleanup per kỳ | Xóa CP/meter/zone → hard delete data kỳ đang mở, giữ data kỳ cũ | Verify data kỳ cũ còn, data kỳ mở mất |
| Xóa zone manager | Xóa đơn vị quản lý khu vực → zones.manager_unit_id = null + cảnh báo | Cảnh báo hiện, manager_unit_id cleared |

### Thao tác đặc biệt (không phải CRUD)

| Thao tác | Trang | Cần kiểm thử |
|---|---|---|
| Tính toán lại (recalculate) | Bảng tính tiền | SA/UA/UA-ZM được, CMD không được. Kỳ đóng → chặn. Kết quả tính đúng |
| Xuất Excel | Bảng tính tiền | Data khớp HTML, formulas đúng, merge đúng, số cột đúng theo role |
| Mở kỳ mới | Đơn giá điện | Chỉ SA. Auto year/month. Snapshot kế thừa đúng. Transaction rollback nếu lỗi |
| Đóng kỳ | Đơn giá điện | Chỉ SA. Cảnh báo mismatch reading_end nếu có |
| Mở lại kỳ cũ | Đơn giá điện | Chỉ SA. Phải đóng kỳ đang mở trước. StructureChangeGuard kích hoạt |
| Backup | Sao lưu dữ liệu | Chỉ TECH. Tối đa 3 bản. pg_dump thành công |
| Restore | Sao lưu dữ liệu | Chỉ TECH. Hệ thống về trạng thái backup |
| Reset mật khẩu | Tài khoản | SA reset cho UA/CMD. TECH reset cho tất cả. force_password_change = true |
| Đổi mật khẩu | /password_change | Mọi user tự đổi. Mật khẩu phải đủ phức tạp (≥ 8, hoa, thường, số, đặc biệt) |
