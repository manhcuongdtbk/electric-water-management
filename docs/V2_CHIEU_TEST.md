...
| C: Kỳ cũ mở lại | Kỳ mở KHÔNG phải kỳ mới nhất | Sửa data mỗi kỳ được | StructureChangeGuard chặn mọi thay đổi cấu trúc |
...
- Data mỗi kỳ đang mở bị hard delete (cleanup callbacks)
...
|---|---|---|---|---|---|
| Sinh hoạt (residential) | Có | Có (personnel_entries mỗi rank) | Có | Tiêu chuẩn + sử dụng + tổn hao + bơm nước | meter_readings + personnel_entries + other_deductions + pump_allocations |
...
| N-2 (cũ mở lại) | N-2 | Data kỳ N-2 | Có | Sửa data mỗi kỳ |
...
Cùng 1 entity trong cùng 1 kỳ, data ban đầu có thể đến từ 3 đường khác nhau. Áp dụng cho TẤT CẢ data mỗi kỳ:

| Data mỗi kỳ | Kế thừa (mở kỳ mới, entity đã tồn tại) | Tạo giữa kỳ (after_create callback) | Kỳ đầu tiên (defaults) |
|---|---|---|---|
...
| ranks | Copy name + quota + position từ kỳ trước | Không áp dụng (rank là cấu hình chung, không theo entity) | 7 ranks mặc định (570, 440, 305, 130, 210, 110, 24) |
...
| Input state | CMD: tất cả input disabled + ẩn nút Lưu. Kỳ cũ mở lại: field cấu trúc disabled, field data mỗi kỳ enabled. reading_start editable mọi kỳ (pre-filled từ kỳ trước nhưng sửa được) | CMD thấy input enabled = bug |
| Dropdown options | Đúng theo role. Zone/unit dropdown: SA thấy tất cả, non-SA không thấy. Loại CP: UA 2 loại (sinh hoạt, công cộng), UA-ZM/SA 4 loại. Kỳ cũ: dropdown hiện entity đã xóa (`with_discarded`) | UA thấy dropdown loại "bơm nước" = bug (chỉ UA-ZM/SA mới thấy) |
| Cảnh báo | Billing: cảnh báo thiếu dữ liệu ("Đơn vị X chưa nhập"), tổn hao bất thường. Dashboard: cảnh báo tương tự. Hiện đúng theo role (SA thấy toàn hệ thống, UA thấy khu vực mình) | SA không thấy cảnh báo khi đơn vị chưa nhập = bug |
| Sidebar | Đúng items theo role (chiều 3 bảng quyền). TECH: 3 items. SA: 17 items. DC: 16 items (tất cả trừ Tài khoản và Sao lưu, chỉ xem). UA: 8 items. UA-ZM: 11 items. CMD: 8 items (khớp UA). CMD-ZM: 11 items (khớp UA-ZM). CMD/DC thấy cùng trang nhưng chỉ xem (inputs disabled, nút ẩn) | CMD thấy nút Sửa/Xóa trên trang chỉ xem = bug |
...
| Tra cứu lịch sử (/history) | Chọn khoảng (range) | Danh sách tổng quan mỗi kỳ trong khoảng |
| Đầu mối (/contact_points) | Lọc theo loại (`type`) | Danh sách chỉ chứa loại đó. Dropdown loại hiển thị đúng theo role (UA: 2 loại, UA-ZM/SA: 4 loại) |
...
|---|---|---|
| Field bắt buộc đầy đủ (name, type, zone/unit) | Redirect index + flash "đã tạo" + record trong DB + data mỗi kỳ tạo đúng (meter_readings, personnel_entries, other_deductions, unit_configs theo chiều 11) | — |
...
| Kỳ cũ mở lại: sửa data mỗi kỳ (reading_end, quân số) | Redirect + DB cập nhật cho kỳ cũ đó, không ảnh hưởng kỳ khác | — |
...
- SA xem billing kỳ N-1: CP đã xóa phải hiện (data mỗi kỳ còn)
...
>
> Từ ADR-030, chiều test mỗi-tính-năng được khai chính thức ở bảng `## Truy vết chiều test` (anchor `CHIEU-<slug>`) của từng spec và CI đối chiếu với test; mục này giữ vai trò catalog 12 chiều khái niệm + trỏ tới spec, không phải nơi theo dõi trạng thái triển khai.
