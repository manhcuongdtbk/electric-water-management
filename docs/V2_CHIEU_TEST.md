...
| Thiết lập | Khu vực (/zones) | CRUD | SettingsAccessGuard (require_system_admin_or_zone_manager!) + PeriodGuard + StructureChangeGuard | CRUD | Xem (khu vực mình) | Chặn | Xem (khu vực mình) | Chặn | Chặn |
...
| Sidebar | Đúng items per role (chiều 3 bảng quyền). TECH: 3 items. SA: 17 items. UA: 8 items. UA-ZM: 12 items. CMD: 8 items (khớp UA). CMD-ZM: 12 items (khớp UA-ZM). CMD thấy cùng trang với UA nhưng chỉ xem (inputs disabled, nút ẩn) | CMD thấy nút Sửa/Xóa trên trang chỉ xem = bug |
