# 02. Bảng thuật ngữ / Glossary — v1.0.0
...
| Tiếng Việt | English (code) | Model / Table | Giải thích |
|---|---|---|---|
| Công tơ | Meter | `Meter` | Đồng hồ đo điện gắn tại một đầu mối. Mỗi đầu mối có ít nhất một công tơ. Có 4 loại (xem bên dưới). Loại được xác định bởi cột `meter_type`. |
...
| Công tơ công cộng | Public meter | `Meter` (meter_type: `public_use`) | Công tơ đo điện dùng chung (hội trường, trường bắn, đèn đường...). **Không xuất hiện** trong bản thu tiền. Vẫn tham gia tính tổn hao. Nghiệp vụ: khi thu mỗi người % công cộng, tiền đó dùng để trả cho các vị trí công cộng này. |
...
| Công tơ vị trí không tổn hao | No-loss meter | `Meter` (meter_type: `no_loss`) | Công tơ đặt tại vị trí trước hoặc tại trạm biến áp — tức điện đo tại đây đã bao gồm tổn hao đường dây rồi, nên phần mềm **bỏ qua** công tơ này khi tính tổn hao phân bổ (tránh tính tổn hao hai lần). |
...
| Chỉ số đầu kỳ | Previous reading | `MeterReading.previous_reading` | Chỉ số công tơ đầu tháng. Tháng đầu tiên: nhập tay. Tháng sau: tự động = cuối kỳ tháng trước. |
| Chỉ số cuối kỳ | Current reading | `MeterReading.current_reading` | Chỉ số công tơ cuối tháng. Admin_unit nhập mỗi tháng. |
| Sử dụng (kW) | Usage / consumption_kw | `MeterReading.consumption_kw` | Lượng điện tiêu thụ = cuối kỳ − đầu kỳ. Phần mềm tự tính. |
| Số điện lực | Electricity supply | — (cần verify: xem TODO) | Số liệu từ đồng hồ tổng: tổng kW điện lực cấp cho đơn vị trong tháng. Admin_unit nhập (F05). Dùng để tính tổn hao = số điện lực − tổng các công tơ. |
...
| 21 | Thừa (kW) | surplus_kw | = cột 19 − cột 20 (chỉ khi dương, tức tiêu chuẩn > sử dụng). Hiển thị màu xanh. Nếu không thừa thì = 0. |
| 22 | Thiếu (kW) | deficit_kw | = cột 20 − cột 19 (chỉ khi dương, tức sử dụng > tiêu chuẩn). Hiển thị màu đỏ. Nếu không thiếu thì = 0. |
| 23 | Thừa (đồng) | surplus_amount | = cột 21 × đơn giá. Tiền thừa (đơn vị dùng ít hơn tiêu chuẩn). |
| 24 | Thiếu (đồng) | deficit_amount | = cột 22 × đơn giá. Tiền thiếu = số phải thu (đơn vị dùng vượt tiêu chuẩn). |
...
| Kế thừa tháng | Month carry-over | — | Khi mở kỳ mới, data tự sao chép từ tháng trước: đầu mối, công tơ, quân số, cấu hình. Admin_unit chỉ cần sửa chỗ thay đổi. Chỉ số đầu kỳ = chỉ số cuối kỳ tháng trước. |
...
| F12 | Dashboard (tháng/quý/năm) | Dashboard | admin_level1, commander (cần verify: xem TODO) | Biểu đồ so sánh tiêu chuẩn vs sử dụng |
...
| `consumption_kw` | Sử dụng (kW) — mức công tơ | 2 |
| `ContactPoint` | Đầu mối | 1 |
| `ContactPointOtherDeduction` | Khoản trừ "Khác" | 4 |
| `current_reading` | Chỉ số cuối kỳ | 2 |
| `deficit_amount` | Thiếu (đồng) — cột 24 | 3.2 |
| `deficit_kw` | Thiếu (kW) — cột 22 | 3.2 |
| `division` | Sư đoàn | 1 |
...
| `Meter` | Công tơ | 2 |
| `meter_type` | Loại công tơ: `normal`, `public_use`, `pump_station`, `no_loss` | 2 |
...
| `Personnel` | Quân số | 1 |
| `previous_reading` | Chỉ số đầu kỳ | 2 |
| `public_kw` | Công cộng — cột 17 | 3.2, 4 |
| `public_use` | Công tơ công cộng | 2 |
...
| `surplus_amount` | Thừa (đồng) — cột 23 | 3.2 |
| `surplus_kw` | Thừa (kW) — cột 21 | 3.2 |
| `tech` | Kỹ thuật | 7 |
...
Các mục dưới đây cần kiểm tra với codebase thực tế và cập nhật glossary khi có kết quả:

1. **"Số điện lực" lưu ở đâu?** — Mục 2 ghi "cần verify". Kiểm tra field/model lưu giá trị số điện lực đồng hồ tổng mà admin_unit nhập qua F05.
2. **F12 Dashboard — admin_unit có xem được không?** — Mục 8.4 ghi "cần verify". Kiểm tra `Ability` class xem admin_unit có quyền truy cập dashboard hay chỉ admin_level1 và commander.
3. **`effective_from` trên RankQuota** — Mục 1 và 9 đã ghi nhận. Cần verify cột này có thực sự tồn tại trong schema hiện tại hay là thông tin từ project knowledge cũ.
4. **Tên cột trong `MonthlyCalculation`** — Mục 3.2 dùng tên English (code) như `surplus_kw`, `deficit_kw`. Cần verify đây là tên cột thật trong database hay tên khác.
