...
**Kết quả Phần B (đã triển khai):** đo 177 nhánh hở, phân loại và xử lý có mục tiêu (KHÔNG đuổi đủ 177 — chỉ vá nhánh nghiệp vụ thật, đánh dấu nhánh chết, *cố ý chừa phần còn lại*):

- **Viết test hành vi thật (7 nhánh):** ma trận ràng buộc theo loại đầu mối trong `app/models/contact_point.rb` — `public` (có nhóm / có quân số → lỗi), `water_pump` (có khối / có nhóm → lỗi), `non_establishment` (zone trống / có khối / có nhóm → lỗi). Test assert **đúng khoá lỗi** (`errors.added?(field, symbol)`), không chỉ `not_to be_valid` — chống vacuous.
- **`# :nocov:` nhánh chết có lý do (2 nhánh, `app/services/summary_calculator.rb`):** `unit_coefficient` trên đầu mối không có đơn vị (chặn sẵn bởi `OtherDeduction#unit_coefficient_requires_unit`) và `else` của `case other_type` (enum chỉ có `fixed`/`coefficient`/`unit_coefficient`). Cả hai là guard phòng thủ giữ lại, không thể chạm tới qua validation.
- **Cố ý chừa (documented, không silent):** các nhánh `&./else` trên association gần như không nil trong vận hành (`period_comparison`, `contact_point#effective_zone`, `pump_allocation`, …), nhánh log trong `rescue` (`zone_warning_collector`), guard cấu hình DB luôn đầy đủ (`backup_service`/`database_config`), và toàn bộ nhánh redirect/guard trong controller — ép test ở đây dễ đẻ test contrived/vacuous, lợi ích thấp. Hai nhánh nghiệp vụ thật còn lại trong `summary_calculator` (đơn vị không có `UnitConfig` — L119; đầu mối không có khoản Khác — L125) **reachable nhưng cần fixture nhẹ riêng** (sample `setup_zone_one_full_sample` seed sẵn config + deduction cho mọi đầu mối) → ứng viên follow-up nếu cần, không làm trong đợt này.
- **Số đo:** trước 762/939 (81,15%); sau **767/935 (82,03%)** branch (line 97,13%). Sàn branch bump **80 → 81** (headroom ~1pt). Line giữ 96.
...
|---|---|---|
| 0.1.1 | 2026-06-14 | Phần B triển khai: +7 test ma trận ràng buộc `contact_point`, `:nocov` 2 nhánh chết `summary_calculator`; branch 81,15% → 82,03%, bump sàn branch 80 → 81; ghi rõ phần cố ý chừa. |
