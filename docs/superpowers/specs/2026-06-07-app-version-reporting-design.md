...
- **Nhãn môi trường là tiếng Anh** (xem ADR-003). Ops đặt `APPLICATION_ENVIRONMENT_LABEL` cho từng môi trường (ví dụ `Acceptance` / `Mirror` / `Production`); trống → `Rails.env.capitalize`.
