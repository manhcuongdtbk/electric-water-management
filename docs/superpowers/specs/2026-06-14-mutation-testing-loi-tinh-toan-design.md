...
> **Cách đọc:** quyết định viết theo **ADR** (Trạng thái → Bối cảnh → Quyết định → Lý do → Tradeoff → Phương án đã loại → Điều kiện xem lại). ADR đánh số toàn cục; spec này thêm **ADR-057**. (Số bị đẩy lên do va chạm liên tiếp với các session song song: 054 ([#332](https://github.com/manhcuongdtbk/electric-water-management/issues/332)/PR #364), 055 (#367), 056 (#374 role-coverage) đều merge `develop` trước — `check-adr-numbering` bắt mỗi lần, renumber dần tới **057**. Bài học: số chưa-merge vô hình với `develop`, nhiều session cùng lúc thì cứ lấy số cao nhất còn trống và để guardrail xác nhận lúc rebase.)
...
- **Dùng gem `mutant`** — công cụ tốt nhất cho Ruby nhưng repo private nên cần subscription trả phí (~250 USD/năm/ghế); owner chưa muốn chi (xem ADR-057 "Phương án đã loại" + "Điều kiện xem lại"). Để dành làm đường nâng cấp.
...
Các class hỗ trợ `ZoneQuery` (gom usage), `LossSnapshotWriter` (ghi snapshot) **để dành mở rộng**, chưa vào phạm vi vòng này (xem ADR-057).
...
Bốn subject lõi (ADR-057), map cứng subject → spec trong một file config (`config/mutation.yml` hoặc hằng trong rake task):
...
Một số đột biến **không đổi hành vi** (vd `x * 1` → `x / 1`, toán hạng trái luôn truthy nên `|| 0` không bao giờ kích hoạt, hoặc nhánh chết) ⇒ sống giả. Danh sách bỏ qua `config/mutation_ignores.yml`: mỗi mục `{file, line, from, to, reason}` — harness loại khỏi tập sinh và đếm vào "ignored". Khớp theo `path/line/from/to`; thêm `column` (0-based, tùy chọn) để khoanh đúng **một** mutant khi một dòng có nhiều đột biến cùng `from->to` (vd hai `+` trên một dòng — một bị giết, một tương đương). Minh bạch, review được; thêm mục là một quyết định có ghi lý do, không phải tắt âm thầm.
...
### ADR-057: Mutation testing lõi tính toán bằng harness tự viết (Ripper), không dùng `mutant` trả phí
...
- **0.3.0 (14/06/2026):** Đóng follow-up **[#376](https://github.com/manhcuongdtbk/electric-water-management/issues/376)** — xử lý trọn 19 survivor còn lại. **12 survivor giết bằng test mới** (biên `c_raw < 0`, `delta > 0`, `total_weighted > 0`; nhánh logic kỳ-đã-đóng `unless closed?`, phân bổ hệ số trực tiếp theo đầu mối `|| 0`; fallback quân số thiếu key dòng 90/105/106/107/137 của `PumpAllocationCalculator`) — dựng fixture biên/thiếu-key chuyên biệt, BigDecimal + ROUND_HALF_UP. **7 survivor là mutant tương đương** (không input nào phân biệt được) → khai `config/mutation_ignores.yml` kèm lý do: `calculation_orchestrator.rb:22` (`+ summary.warnings` luôn `+ []`), `summary_calculator.rb:78` (`>`→`>=` trùng tại delta=0), và 5 fallback `|| 0`/`else 0` mà toán hạng trái luôn là Integer truthy (`Hash.new(0)` / cache luôn set) hoặc nhánh chết do ràng buộc XOR của `PumpAllocation`. **Baseline mới: 107 mutant catalog → 7 ignored, 100 killed, 0 survived** (`rake mutation:core` exit 0). Tinh chỉnh harness: `ignore_key` thêm `column` (tùy chọn) để khoanh đúng một mutant khi một dòng có nhiều đột biến cùng `from->to` (ca `calculation_orchestrator.rb:22` có hai `+`, một bị giết một tương đương). Không cần ADR mới — thực thi tiếp ADR-057.
- **0.2.1 (14/06/2026):** Đổi số ADR **056 → 057** khi rebase lên `develop`: 056 đã bị #374 (role-coverage) chiếm. Chỉ đổi số, nội dung quyết định giữ nguyên.
- **0.2.0 (14/06/2026):** Triển khai xong + baseline lần đầu. `rake mutation:core` trên 4 subject lõi: **107 mutant, 88 killed, 19 survived, 0 ignored** (81% killed). Đã giết survivor giá-trị-cao `summary_calculator.rb:88` (`surplus_amount = surplus * unit_price`, dòng tiền thừa trước đó chưa được assert) bằng một assertion ở `summary_calculator_spec.rb`. 19 survivor còn lại (biên cần fixture edge; logic cảnh báo/kỳ-đóng; fallback `|| 0`/`.sum(0)` quân số) tách follow-up **Issue #376** (không phình PR — đúng DoD). ADR-057.
- **0.1.0 (14/06/2026):** Bản đầu — thiết kế harness mutation testing tự viết (Ripper, stdlib) cho 4 class lõi tính toán; catalog toán tử nhắm lỗi tài chính; chạy định kỳ `rake mutation:core`; ignore-list mutant tương đương; đường nâng cấp `mutant` để ngỏ. ADR-057, Issue #358.
