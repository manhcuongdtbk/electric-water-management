...
- **Truy vết:** GitHub Issue [`#334`](https://github.com/manhcuongdtbk/electric-water-management/issues/334), milestone `1.2.0`, nhãn `priority-high`. Phát sinh từ review TN3 ([`#331`](https://github.com/manhcuongdtbk/electric-water-management/issues/331)).
- **Trạng thái khách:** vấn đề nền tảng/an toàn dữ liệu; nghiệm thu khi dùng thử trên Acceptance sau triển khai.
- **Khách-thấy-được / demo:** thay đổi này **customer-facing** (chỉ báo độ tươi hiện trên 5 trang + hộp xác nhận khi xuất Excel cũ + dòng cảnh báo đóng dấu trong file). Vì vậy PR gắn nhãn `customer-facing` và **kèm một demo spec** theo ADR-040 (xem mục Demo). Quyết định này do session #334 chốt theo ủy quyền ở comment Issue #334 (đính chính 2026-06-13).
...
## ADR-049: Chỉ báo độ tươi per-khu-vực qua bảng trạng thái `calculation_states` + guard xuất Excel ba lớp
...
| `CHIEU-do-tuoi-excel-confirm` | Lớp UX (Stimulus): bấm Xuất Excel khi stale → hộp xác nhận tiếng Việt; huỷ → không tải file (system spec) | có test |
...
Thay đổi customer-facing ⟹ PR gắn nhãn `customer-facing` + ship một demo spec dưới `spec/demo/` (vd `freshness_demo_spec.rb`, `type: :demo`, dùng `DemoRecorder` + caption tiếng Việt + seed `db/seeds/demo.rb`). Kịch bản kể-chuyện một mạch:

1. Đăng nhập, mở Bảng tính tiền một khu vực đã tính → trạng thái "đã tính & còn đúng".
2. Sang trang nhập chỉ số, sửa một chỉ số đầu mối → quay lại billing/Tổng quan → **badge "cần tính lại"** hiện đúng khu vực đó.
3. Thử **Xuất Excel** khi đang cũ → hộp **xác nhận cảnh báo**; nếu vẫn xuất → file có **dòng cảnh báo đóng dấu**.
4. Bấm **"Tính toán lại"** → badge biến mất (về "còn đúng"); xuất Excel sạch, không còn cảnh báo.

Demo minh hoạ trực tiếp tính năng an toàn (đóng kịch bản nguy hiểm Chiều 8). Guardrail `check-demo-spec.sh` xanh nhờ PR có `customer-facing` chạm `spec/demo/**`.
...
- Đồng bộ dòng truy vết: milestone **1.3.0 → 1.2.0**. Sau khi #334 merge vào `develop` (PR #366), gate người chốt đưa vào release 1.2.0 (1.2.0 chưa cắt release nên #334 đã trên develop sẽ ship cùng); giữ entry 1.0.0 nguyên làm lịch sử quyết định triage ban đầu.

### 1.2.1 (2026-06-14)

- Đổi số ADR **048 → 049** để tránh trùng: sau khi tích hợp `develop` mới nhất, #351 (đã merge) đã chiếm ADR-047 và ADR-048. ADR-049 là số trống. (Guardrail check-adr-numbering ép duy-nhất, không ép liên-tục.)

### 1.2.0 (2026-06-14)

- Thêm chiều test `CHIEU-do-tuoi-excel-confirm` cho lớp UX Stimulus (hộp xác nhận khi xuất Excel cũ) — phát sinh khi viết plan để mỗi lớp guard có anchor riêng, khớp test-dimensions (ADR-030).

### 1.1.0 (2026-06-14)

- Chốt #334 là **customer-facing** (theo ủy quyền ở comment Issue #334): PR gắn nhãn `customer-facing` + kèm demo spec (ADR-040). Thêm mục **Demo** (kịch bản) và ghi chú khách-thấy-được. Anchor demo `CHIEU-do-tuoi-5-trang`/`-excel-*` đã có.

### 1.0.0 (2026-06-14)

- Bản thiết kế đầu tiên (ADR-049): chỉ báo độ tươi dữ liệu dẫn xuất per khu vực-kỳ qua bảng `calculation_states` (marker bump bắt cả hard-delete) + guard xuất Excel ba lớp. Brainstorm chốt hướng từ đề xuất mở của Issue #334 (milestone 1.3.0, priority-high).
