...
| **Mutant tương đương** (equivalent mutant) | Mutant **không đổi hành vi** quan sát được (vd `x * 1`→`x / 1`, hoặc nhánh chết) nên *luôn* sống mà không phải lỗi test. Loại khỏi báo cáo qua danh sách bỏ qua có lý do (`config/mutation_ignores.yml`, ADR-056) — minh bạch, không tắt âm thầm. |

## Lịch sử thay đổi

- **1.10.0 (14/06/2026):** §3 thêm 3 gloss cho mutation testing lõi tính toán (ADR-056, Issue #358): **"Mutation testing"** (kiểm thử đột biến — đo độ mạnh test, khác coverage), **"Mutant"** (bản code đổi đúng một chỗ; killed/survived), **"Mutant tương đương"** (không đổi hành vi, loại qua ignore-list có lý do). Không đăng ký vào `glossary-terms.txt` (giữ baseline guardrail ADR-024).
